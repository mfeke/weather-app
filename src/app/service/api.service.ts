...
export class ApiService {
  ...
  getCurrentWeather(lat: any, lon: any): Observable<any> {
    return this.httpClient.get<any>(
      `${this.baseURL}?lat=${lat}&lon=${lon}&appid=${this.API_key}&units=metric`
    );
  }
  isGetBackground(search:any): Observable<any> {
    return this.httpClient.get<any>(
      `${this.url}?query=${search}&client_id=Te0RIurBP_5TsZ9kuTgZsy9N1XL-TALb12_dkEB3QGY`
    );
  }
  ...
  getHours(lat: any, lon: any): Observable<any> {
    ...
    return this.httpClient.get<any>(
      `${url}?lat=${lat}&lon=${lon}&appid=${this.API_key}`
    );
  }
